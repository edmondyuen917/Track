...
res_x = x_size - x_margin * 2
res_y = y_size - y_margin * 2
latitude_column = 6
longitude_column = 7
max_x = max_y = -1
min_x = min_y = 100000
...
with open(file_out, 'w') as write_obj:
    for row in list_of_rows:
        ...
        x = float(row[longitude_column])
        if ((x != 200) and (y != 200)):
            x = (round(((x - min_x) / delta_x)*res_x))
            y = (round(((y - min_y) / delta_y)*res_y))
...
plt.grid(True)
plt.text(0, 0, "total segment: " +str(len(new_array)), fontdict = font)
plt.text(0, 20, "X:" +str(ThresholdX) + " " + tempStr + " Y:" +str(ThresholdY), fontdict = font)
